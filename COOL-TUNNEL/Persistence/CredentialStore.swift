...
//
// Common interface for per-profile credential storage so the higher
...
//
// **v3.0.0 (sub-phase F):** the stored credential is a VLESS UUID,
// not a NaiveProxy basic-auth password. Method names rename
// `password` → `uuid` in lock-step with `Profile.uuid` so the
// boundary between the in-memory profile and the persistent
// keychain/file store stays self-documenting. The semantics
// (empty string == not stored, item-not-found returns `""`, throw
// on backend failure) are preserved verbatim.
//
...
//
// The migrating store is the one the orchestrator wires up. For v3.0.0
// the legacy leg is a freshly-constructed `KeychainStore` on the new
// `…vless-credentials` service identifier — the v2.x
// `…proxy-credentials` items are orphaned by design (those are
// basic-auth passwords that would mis-shape a VLESS UUID parse).

import Foundation
import os

/// Per-profile credential backend. Implementations must be safe to
/// call from any actor — the orchestrator hits them on its main
/// actor but tests run them off-thread.
public protocol CredentialStore: Sendable {
    /// Returns the stored UUID for a profile id, or an empty
    /// string when nothing is stored. Throws on unexpected backend
    /// failures (a missing item is **not** an error).
    func uuid(forProfileID id: String) throws -> String

    /// Writes (or overwrites) the UUID for a profile id. Empty
    /// strings delete the entry — there is no point in keeping a
    /// sentinel "no credential" record.
    func setUUID(_ uuid: String, forProfileID id: String) throws

    /// Removes the entry for a profile id. No-op when none is stored.
    func deleteUUID(forProfileID id: String) throws
}
...
/// **the user takes an action that needs the credential** (clicking
...
public struct MigratingCredentialStore: CredentialStore {
    ...
    public func uuid(forProfileID id: String) throws -> String {
        let primaryValue = try primary.uuid(forProfileID: id)
        ...
        // credential (e.g. user clicked Start).
        // try-ok: keychain locked / dismissed prompt → re-prompt user, no data loss
        let legacyValue = (try? legacy.uuid(forProfileID: id)) ?? ""
        ...
        // legacy delete would lose the credential entirely. Now if
        // the promotion fails, the legacy copy stays put and the
        // user keeps their credential — the worst case is one more
        // Keychain prompt on next launch, not data loss.
        let promoted: Bool
        do {
            try primary.setUUID(legacyValue, forProfileID: id)
            promoted = true
        } catch {
            promoted = false
        }
        if promoted {
            ...
            // backends.
            do {
                try legacy.deleteUUID(forProfileID: id)
            } catch {
                Self.logger.info(
    ...
    public func setUUID(_ uuid: String, forProfileID id: String) throws {
        try primary.setUUID(uuid, forProfileID: id)
        // Also clear the legacy entry on a fresh write — keeps the
        // two backends from drifting if the user changes a credential
        ...
        do {
            ...
            Self.logger.info(
                "legacy keychain cleanup after setUUID failed for profile \(id, privacy: .public): \(error.localizedDescription, privacy: .public); primary holds the new value"
            )
        }
    }

    public func deleteUUID(forProfileID id: String) throws {
        try primary.deleteUUID(forProfileID: id)
        // **M1 (v2.0.38):** log on legacy-cleanup failure.
        do {
            try legacy.deleteUUID(forProfileID: id)
        } catch {
            Self.logger.info(
                "legacy keychain cleanup after deleteUUID failed for profile \(id, privacy: .public): \(error.localizedDescription, privacy: .public); primary already deleted"
            )
        }
    }
