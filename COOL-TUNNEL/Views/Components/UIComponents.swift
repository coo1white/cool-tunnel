...
#Preview("VerdictPill") {
    VStack(alignment: .leading, spacing: 6) {
        VerdictPill(
            kind: .ok,
            tag: "OK",
            message: "Ready to use · universal · v138.0.7204.157",
            systemImage: "checkmark.circle.fill"
        )
        VerdictPill(
            kind: .ng,
            tag: "NG",
            message: "Missing arm64 slice — proxy will fail to spawn.",
...
#Preview("SummaryRow") {
    VStack(alignment: .leading, spacing: 4) {
        SummaryRow(
            label: "Path",
            value: "/Applications/Cool Tunnel.app/Contents/Resources/naive",
            monospaced: true
        )
        SummaryRow(
            label: "Architectures",
        ...
        SummaryRow(label: "Version", value: "138.0.7204.157")
    }
    .padding(20)
    .frame(width: 480)
}
