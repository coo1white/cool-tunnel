...
- **VLESS + Reality payloads.** Cool Tunnel hands inbound traffic to the bundled `sing-box` binary via a SOCKS5 listener on `127.0.0.1`. `sing-box` forwards through a VLESS outbound (with Reality SNI cover) to the operator-controlled VPS. The Cool Tunnel app (Swift UI + Rust core) **never decrypts, parses, or stores** anything inside that tunnel. JSON-RPC bodies, signed transactions, wallet addresses, and method names exist only inside the encrypted payload between user's browser/wallet and destination.
- **Destination URL of routed traffic.** Data plane delivers TLS bytes; the app does not log per-request destinations. System proxy routes to `127.0.0.1:<port>`; from `sing-box`'s view the destination is visible (SOCKS5 target), but `sing-box`'s default verbosity is quiet — logs errors, not per-request URLs.
...
- **Process arguments that expose credentials.** `sing-box` is spawned with argv `[<binary>, run, -c, <config-path>]` only. VLESS UUID and Reality keys live inside the 0o600 config file; they never appear in `ps`, in stdout/stderr, in OS process listings, or in audit-trail tools that read argv.
...
Three pieces of state under `~/Library/Application Support/COOL-TUNNEL/`:

1. **`config.json`** (mode 0600). Carries the generated sing-box config — server host, port, VLESS UUID, Reality `public_key` / `short_id` / `dest_host`. Generated fresh per Start, atomically written via `RestrictedFile.write` with `O_CREAT|O_EXCL` (no umask race). Deleted on graceful Stop. Time Machine excluded via `setResourceValues(.isExcludedFromBackup)` on parent directory.
2. **`credentials.json`** (mode 0600). Profile credentials. Same atomic-write discipline. Same Time Machine exclusion.
...
A PAC file (mode 0600) is also written when Smart mode is active. Contains user's direct-domain list plus SOCKS listener address (`127.0.0.1:<port>`). No user destinations, no operator credentials.

## Known surfaces — places where information CAN leak

**Live log view (Log Console in the UI).** Surfaces `sing-box` stdout/stderr in real time. Credential-shaped strings — UUIDs, Reality keys, `Authorization:` and `Cookie:` headers — are filtered through `cool_tunnel_core::redaction::redact` (regression-tested) before the line crosses the supervisor boundary. **However: destination hostnames in `sing-box` error messages CAN reach the log.** An error line might say `"vless-out connect to alchemyapi.io:443 failed"` — leaking that the operator was tunneling Alchemy traffic, even though no payload bytes are exposed. Redaction strips credentials, NOT destination identifiers. Operator guidance: treat the live log as sensitive; don't screenshot it during an RPC-tunneling session; clear before sharing a support transcript.

**Lifecycle telemetry file.** Same redaction discipline as live log. Same caveat: destination hostnames in an error message embedded in `error.localizedDescription` could pass through. File is 0o600, in user's home Application Support, Time Machine excluded — but anyone with read access to the user account can read it. Delete it if a support session ends or it accumulated content from an RPC-tunneling session you don't want preserved:

```sh
rm "$HOME/Library/Application Support/COOL-TUNNEL/lifecycle-telemetry.jsonl"
```
...
**Updater traffic.** `AppUpdater` / `SingboxUpdater` / `RustCoreUpdater` issue HTTPS to `api.github.com` and `*.githubusercontent.com`. Host suffix list enforced by `GitHubTrust` (regression-tested in `GitHubTrustTests`); redirects to non-trusted hosts rejected before download. Checks happen on explicit user click; no background polling. GitHub learns: that an instance of Cool Tunnel checked for updates at a given time, from a given IP. Same metadata Homebrew, `npm`, `pip`, `cargo`, `gh` emit. User identity not sent; User-Agent is `URLSession`'s default.
