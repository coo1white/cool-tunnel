...
//
// scripts/preflight.ts — local CI floor.
//
...
async function main(): Promise<void> {
    ...
    // default $PATH, which made the lint job silently no-op.
    if (
        !(await runCheck({
    ...
    // Only the Xcode build helper remains shell; maintenance commands
    // live in Bun scripts and are covered by `bun test`.
    const shellFiles = [
        join(root, "scripts", "build_rust_core.sh"),
        join(root, "bin", "ct"),
    ];

    if (
        !(await runCheck({
            label: "shellcheck scripts/build_rust_core.sh bin/ct",
